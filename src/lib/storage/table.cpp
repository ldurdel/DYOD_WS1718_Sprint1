...
uint64_t Table::row_count() const {
  return std::accumulate(_chunks.cbegin(), _chunks.cend(), 0,
                         [](auto acc, const auto& chunk_ptr) { return acc + chunk_ptr->size(); });
}
...
void Table::compress_chunk(ChunkID chunk_id) {
  Assert(chunk_id < chunk_count(), "Index out of bounds");
  Assert(chunk_id < chunk_count() - 1, "The last chunk must not be compressed");

  const Chunk& uncompressed_chunk = get_chunk(chunk_id);
  auto compressed_chunk = std::make_shared<Chunk>();

  for (auto column_index = ColumnID{0}; column_index < uncompressed_chunk.col_count(); ++column_index) {
    const auto& column_type = _column_types.at(column_index);
    const auto& uncompressed_column = uncompressed_chunk.get_column(column_index);
    auto compressed_column = make_shared_by_column_type<BaseColumn, DictionaryColumn>(column_type, uncompressed_column);
    compressed_chunk->add_column(compressed_column);
  }
